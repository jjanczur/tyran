...
description: 'What the work cost, in tokens the platform itself reported: per model, per agent type and per ticket, priced by a rate card you write.'
---
...
[Projections](../projections/) states the same boundary from the other side.
...
4. **A resumed headless session still counts.** [Overnight mode](../overnight/)
