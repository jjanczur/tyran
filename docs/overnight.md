...
1. **Telemetry, from either of two channels.** The hook payload carries no
...
`warn` surfaces through doctor and never denies. The default is `off` because
it is a behaviour change an operator should choose, not because it needs a
setup step: enabling it takes one edit and the Settings tab will make it.
`limit-telemetry-missing` now fires only when NO channel is reachable, which
