...
| `mistakes-file-missing` | info | there is no `MISTAKES.md` **and git has never seen one** — the repository predates the ledger rather than having opted out of it. A deletion is the documented opt-out and produces NO finding, and where git cannot answer at all this says nothing rather than guessing. Info, because nothing is broken: it is an offer, not a defect ([self-improvement](self-improvement.md)) |
