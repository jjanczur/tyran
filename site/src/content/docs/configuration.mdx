...
out in full so [the Settings tab](../board/#settings) can edit it, because that
...
1. **Secret scanning.** `gitleaks` runs from the
   [secrets gate](../hooks/#the-secrets-gate-and-exactly-where-it-stops), a different hook that never reads
