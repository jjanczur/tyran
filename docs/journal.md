...
`scripts/journal.mjs` · 84 unit tests
...
contents are moved under `state/`. `scripts/migrate.mjs` does that move:

```bash
node scripts/migrate.mjs --dir .tyran            # what it WOULD move
node scripts/migrate.mjs --dir .tyran --apply    # move it
```

Explicit and never automatic, because it MOVES an append-only history rather
than seeding a file that does not exist yet. It never overwrites — a name that
already exists under `state/` is reported as a conflict, skipped, and left
untouched on both sides, because merging two directories that share a name is
a decision only the operator can make. It never deletes: the legacy directory
goes only once it is empty. It is idempotent, so re-running after fixing a
conflict by hand is safe, and it exits 1 while any conflict remains so an
automated caller can tell "done" from "done except for the part that mattered".
...
| `finding` | `area`, `claim` | a claim about one `area`, with its proof, queryable by other agents (+ `proof`, `command`, `exit_code`, `ticket`, `confidence`; `append` issues `F-<n>` ids) |
...
material belongs in `NOTES.md` with a reference here. A finding's `command` is
capped tighter, at 500: measured across this repo's own documentation, 86 shell
commands have a median length of 74 codepoints and a longest of 123, so 500
passes any real pipeline while refusing pasted OUTPUT, which belongs in the
report rather than the journal. `validateJournal`
...
node scripts/journal.mjs ask         <file> <init> --question Q [--recommendation R] [--default D] [--ticket T] [--actor A] [--blocking]
```
...
plus `question`, `recommendation`, `default`, `ticket` and `blocking` in
`data`:
...
**A recommendation becomes the `default` when you name no other**, at the mint
rather than at every read — so an ask carrying one is a question with a safe
outcome, and the board's `blocking · no safe default` split keeps meaning
"neither was supplied". `--blocking` marks the asks that must wake a human even
under `unattended.mode: on` (see [overnight mode](overnight.md)); it is written
only when true, so an ordinary ask is unchanged on disk.
...
The operator answers through [the sheet](./board.md#answering), which appends
