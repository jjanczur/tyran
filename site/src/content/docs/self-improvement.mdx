...
<Verdict state="shipped" /> `agents/retro.md` carries the curator and its filter,
`skills/retro/SKILL.md` runs it, and a `Stop` gate (`hooks/scripts/retro-gate.mjs`) refuses
one turn when an initiative ends without a retrospective. Still outstanding: the update
delta-review that reconciles a new core version with what your repo has learned.
</Aside>
