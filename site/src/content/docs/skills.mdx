---
title: 'What Tyran ships: fourteen skills and five agents'
description: 'The fourteen skills and five agents the plugin installs, each with its role, the assumption it rests on, and whether a person or the conductor normally invokes it.'
---
...
| [`code-review`](https://github.com/jjanczur/tyran/blob/main/skills/code-review/SKILL.md) | The depth half of a review: the dimensions a diff is read against, and the rule that you try to refute a finding before you report it. | A diff and the acceptance criteria it claims to satisfy. It does **not** own the verdict — that stays with `tyran:reviewer`, so the two never define "reviewed" twice. | Either |
| [`root-cause`](https://github.com/jjanczur/tyran/blob/main/skills/root-cause/SKILL.md) | Work a failure down to its mechanism instead of patching the symptom: reproduce, one variable per experiment, bisect, name the mechanism. | That the failure can be made to happen at least sometimes. Everything downstream depends on a reproduction; without one, a fix cannot be shown to have worked. | Either |
| [`browser-check`](https://github.com/jjanczur/tyran/blob/main/skills/browser-check/SKILL.md) | Drive a real browser and return counts — pages, links, console errors, failed responses, computed styles — rather than an impression. | A browser (chromium) and a server that is actually serving the build you mean. It measures; it does not judge whether the design is right. | Either |
| [`deslop`](https://github.com/jjanczur/tyran/blob/main/skills/deslop/SKILL.md) | The optimization pass, defined: delete before you add, one smell class per pass, behaviour pinned by a test. Also lints a `SKILL.md` and prose. | A test that ran **before** the edit. Without a green run from beforehand, a green run afterwards proves nothing about what you changed. | Either |
| [`pr-feedback`](https://github.com/jjanczur/tyran/blob/main/skills/pr-feedback/SKILL.md) | Work a reviewer's comments to the end: all three of GitHub's comment surfaces, a disposition for every comment, push before you reply. | A pull request and `gh` authenticated against it. Assumes the reviewer is a person whose disagreement is worth a written answer, not a queue to clear. | Either |
