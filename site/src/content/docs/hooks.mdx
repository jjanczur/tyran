---
title: 'Hook runtime reference'
description: 'The runtime every Tyran hook runs inside: gates versus probes, what the deadline actually promises, the secrets gate, and the measured limits of each.'
---
...
| file | job |
|---|---|
| [`hooks/scripts/hook-io.mjs`](https://github.com/jjanczur/tyran/blob/main/hooks/scripts/hook-io.mjs) | the runtime every hook runs inside |
| [`hooks/HOOK-CONTRACT-MEASURED.md`](https://github.com/jjanczur/tyran/blob/main/hooks/HOOK-CONTRACT-MEASURED.md) | what the platform actually does, read out of the shipped binary |

Gates shipped so far: [the evidence gate](../evidence-gate/) (`SubagentStop`),
the secrets gate (`PreToolUse`, below),
[the policy gate](../policy-gate/) (`PreToolUse`) — the autonomy classes and
the deployment class, plus the one rule that guards a *read* — and
[the usage gate](../overnight/) (`PreToolUse`) — the wind-down near the
...
The same split, with the controls this repo ships hung off the events that
dispatch them:

```mermaid
flowchart LR
  subgraph GATES["Gates — refusal is the product (can refuse)"]
    direction LR
    PTU(["PreToolUse"])
    SAS(["SubagentStop"])
    PRC(["PreCompact"])
    ...
    TCM(["TaskCompleted (team mode only)"])

    PTU --> SEC["secrets gate<br/>Bash"]
    PTU --> POL["policy gate<br/>every tool"]
    PTU --> WRG["write guard<br/>Write · Edit · NotebookEdit · Bash"]
    SAS --> EVG["evidence gate"]
    PRC --> PCP["pre-compact<br/>writes a checkpoint"]
  end

  subgraph PROBES["Probes — inject or record (cannot refuse)"]
    direction LR
    SST(["SessionStart"])
    SBS(["SubagentStart"])
    PST(["PostToolUse"])
    ...
    SST --> SSP["session-start<br/>re-injects the checkpoint"]
  end
```

Events drawn with no control attached are ones the platform offers and this
repo has not registered anything on.
...
- **you cannot approve by accident.** `PASS` means "no objection" and emits
  ...
  **auto-approves the call and skips the permission prompt** — so it is
  ...
  ([policy-gate](../policy-gate/#turning-the-gate-down)). `PASS` never becomes
...
- **on `SubagentStop`, read `stop_hook_active` and pass when it is true.** It
  ...
  own code. [`evidence-gate.mjs`](https://github.com/jjanczur/tyran/blob/main/hooks/scripts/evidence-gate.mjs) is the
...
<Aside type="danger" title="Writing earlier does not help">
A watchdog that emits the refusal sooner
...
<Limit title="Not a guarantee that a secret cannot be published" kind="floor">
...
<Measured from="7168 real Bash commands from this project's own transcripts">
...
<Limit title="A denylist over shell text" kind="floor">
