...
- **you cannot approve by accident.** `PASS` means "no objection" and emits
  `{}`. `permissionDecision: "allow"` does not mean "satisfied" — it
  **auto-approves the call and skips the permission prompt** — so it is
  reachable only from a handler that returns `{decision: 'allow'}` on purpose,
  which one gate does only when a repo sets `boundaries.prompts: skip`
  ([policy-gate](policy-gate.md#turning-the-gate-down)). `PASS` never becomes
  `allow`, and no failure path does either: see the next bullet, which is the
  half that matters;
...
- **The payload is capped at 10 MiB** (`MAX_PAYLOAD_BYTES`, 10,485,760 bytes)
  and a larger one refuses rather than being scanned in part. A partial scan
  that reports nothing looks exactly like a clean one.
  ...
  4 MiB / 3.14 s against a 7 s deadline — so 4 MiB fit and 8 MiB never did.
  ...
  The cap went from 4 MiB to 10 MiB in 0.1.47 **together with the time it
  buys**, not instead of it: `DEADLINE_MS` 7 s → 15 s, the scanner's own
  budget 5 s → 11 s and the registered `timeout` 15 s → 30 s, the most a
  hook here may declare. The reason was a real project whose two 2.4 MB
  translation catalogs plus the code that used them made one finished
  change, and the gate made it three commits and a chunked push. Measured
  end to end through the real hook on the same
  machine (8 cores, load average 3): copies of a 2.4 MB JSON catalog,
  4 MiB / 3.14 s, 8 MiB / 5.99 s, 9.5 MiB / 7.45 s; base64 folded at 76
  columns, 9.9 MiB / 6.97 s. The ceiling counts every object git would write,
  so 9.9 MiB of catalog files arrived as 10,703,651 bytes in nine objects and
  was refused — in 0.08 s, since the size is read before any object is.

  A cost that only appeared with size: gitleaks 8.30.1 stops reading one
  line that has no newline before the end, deterministically for a given
  payload and chunking, exits 0 and reports the smaller count. Measured: 3 of
  18 runs at 8 MiB of unbroken base64, 0 of 6 at 4 MiB, never once the same
  bytes carried a newline every 76 columns. The coverage check turns that
  into a refusal (*the scanner read N of the M bytes*), which is the side to
  fail on; a source file already has newlines, and a blob that does not is
  the kind of file the remedy says to keep out of the commit.
